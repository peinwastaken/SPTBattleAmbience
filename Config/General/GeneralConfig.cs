...
namespace SPTBattleAmbience.Config.General
{
    public static class GeneralConfig
    {
        ...
        public static void Bind(ConfigFile config)
        {
            ...
            AmbientRolloff = config.Bind(category, "Ambient Rolloff Distance", 4000, new ConfigDescription("Rolloff distance for ambient battle sounds", null, new ConfigurationManagerAttributes { Order = 980 }));
            OcclusionTestMode = config.Bind(category, "Occlusion Test Mode", EOcclusionTest.ContinuousPropagated, new ConfigDescription("Occlusion test mode for ambient battle sounds", null, new ConfigurationManagerAttributes { Order = 978, IsAdvanced = true }));
